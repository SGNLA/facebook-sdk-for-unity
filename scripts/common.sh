...
downloadUnityJarResolverFromGithub() {
  ...
  $UNITY_PATH -quit -batchmode -logFile -importPackage "$UNITY_PACKAGE_PATH" -projectPath="$PROJECT_ROOT/UnitySDK" || die "Failed to import $UNITY_PACKAGE_PATH"
  info "Cleaning up..."
  rm "$UNITY_PACKAGE_PATH"
  mv $PROJECT_ROOT/Examples $PROJECT_ROOT/UnitySDK/Assets/FacebookSDK/
  popd > /dev/null
}
